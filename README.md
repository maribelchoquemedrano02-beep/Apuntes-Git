...
### ¿Cómo nació GIT?
Fue creado por Linus Torvals porque BitKeeper le retiro la licencia por incumplir la regla de "No usaras ni tu ni tus colaboradores otro además de mi", creando git en 2 a 3 semanas.
### ¿Como instalar Git?
...
- Un README.md es una documentación de tu proyecto.
- .gitignore es un archivo de texto para ignorar archivos sensibles que tienes. 
...
Aqui Git observa nuestros archivos y los catáloga en dos opciones:
- Untracked: cuando esta sin seguimiento
- Modified: cuando GIT ya tiene la version modificada
...
Si no quieres que GIT vea tu archivo creado lo que tienes que haces es poder tu archivo al .gitignore.
...
### Buenas prácticas para hacer un commit
Los commits solo tienen que describir un solo cambio pequeño(commits atómicos)
- Usar verbos imperativos como: Add, Change, Fix y Remove.
- No tenemos que usar punto al final y tampoco puntos suspensivos.
- Usar como máximo 50 caracteres
- Usar prefijos para que sea mas legibles como: feat, fix, perf, build, ci, docs, refactor, style y test.
- Poner un contexto después del commit en su cuerpo pero solo con informacion necesaria.

## Clase 3
### GITHUB Y SSH
#### ¿Qué es GITHUB?
Es una red social donde gestionamos y colaboramos en proyectos con otros desarrolladores usando GIT.
La diferencia entre GIT y GITHUB es que que el primero controla las versiones que crea el punto de guardado y el segundo es donde se almacena estos puntos de guardados y se comparte con el mundo.
#### SSH vs HTTPS
HTTPS este nos pide autentificación cada vez, cuando clonamos y queremos usar el repositorio,
pero el SSH (recomendado usar) se configura con una clave que al ponerla en GITHUB no nos pide más autentificación.
#### Para configurar SSH
...
2. Pones un nombre y una descripción y haces clik en "crear repositorio"
...
- git remote set-url <apodo> "url" : para cambiar la url de donde apunta nuestro repositorio.
#### Multiples SSH
Cuando tenemos otras cuentas en github o simplemente necesitamos más cuentas tenemos que tener más de una llave SSH.
Para configurar los multiples SSH seguimos estos pasos:
1. Generamos el ssh key con otro nombre: 
- ssh-keygen -t ed25519 -C "nuestroCorreo" -f~/.ssh/id_miname
...
de un commit antiguo, recuperar los archivos borrados o cambiados, probar cambios sin llegar ha arruinar la rama principal y para saltar de una rama a otra.
El estado "Detached HEAD" aqui lo normal es que el HEAD apunte a una rama en movimiento pero en su lugar apunta a un commit fijo.
...
1. No trabajes por largo tiempo en "Detached HEAD", si ecribes más de una linea trabaja en otra rama.
...
### ¿Qué son las ramas?
Las ramas son las principales utilidades para poder tener un mejor control del codigo que escribimos. Esto trata de dividir la rama principal (main o marter) para que podamos trabajar en nuevas características y correcciones.
